<template>
  <div>
    ...
    >
      <template #[NEST_CHECKED_LEFT]="{ record }">
        <span class="handle">
          <SwapOutlined />
        </span>
      </template>
      <template #[NEST_CHECKED_RIGHT]="{ record }">
        <MoreOutlined />
      </template>
      <template #type="{ record, text }">
        {{ text === "C" ? "菜单" : "按钮" }}
      </template>
      <template #operate="{ record, text }">
        <button>操作</button>
      </template>
    </NestTable>
  </div>
...
import NestTable from "./NestTable.vue";
...
const move = (evt) => {
  console.log("evt: move", evt.related);
};
